...
message_maker(#state{actor=Actor}) ->
    fun(Key, {CRDT, VV, DeltaBuffer}, _NodeName) ->
...
message_handler({_, vector, _, _, _}, _State) ->
    fun({Key, vector, From, Bottom, RemoteVV}) ->

        %% store it, in case it's new
        Default = get_entry(Bottom),
        {ok, {_, _, DeltaBuffer}} = ldb_store:update(
            Key,
            fun(Stored) -> {ok, Stored} end,
            Default
        ),
...
message_handler({_, dotted_buffer, _}, _State) ->
...
init([]) ->
    ...
    lager:info("ldb_vanilla_scuttlebutt_backend initialized!"),
...
handle_call({update, Key, Operation}, _From, #state{actor=Actor}=State) ->
    ...
    Function = fun({{Type, _}=CRDT0, VV0, _}=StoreValue0) ->
        case Type:delta_mutate(Operation, Actor, CRDT0) of
            {ok, Delta} ->
                Dot = vclock:next_dot(Actor, VV0),
                StoreValue = store_delta(Dot, Delta, StoreValue0),
                {ok, StoreValue};
            Error ->
                Error
        end
